...
figureFileNameMaster = "./NREL_FXXXXX_5D_000_00025_copy/exportedCSVMean/";
...
coordinate_yPlaneMean.xq = xq;
coordinate_yPlaneMean.zq = zq;



save("./NREL_FXXXXX_5D_000_00025_copy/exportedCSVMean/coordinate_yPlaneMean.mat", ...
        "coordinate_yPlaneMean");


%% interp data to uniform grid

% indexOfCSVfileArray = 0:1:2400;
indexOfCSVfileArray = 0:1:0;
...
for indexCSVHolder = indexOfCSVfileArray
    ...
    csvTail = {};
    csvTail{1} = "./NREL_FXXXXX_5D_000_00025_copy/exportedCSVMean/yPlane_00D_mean.csv";
    ...
    openFaomUMean = cell(fileNumber,numberOfPlanes);
    openFaomVMean = cell(fileNumber,numberOfPlanes);
    openFaomWMean = cell(fileNumber,numberOfPlanes);
    ...
    for pp = 1:fileNumber
        ...
        for gg = 1:numberOfPlanes
            ...
            openFaomUMean{pp, gg} = dataIn{pp, gg}.("UMean:0");
            openFaomVMean{pp, gg} = dataIn{pp, gg}.("UMean:1");
            openFaomWMean{pp, gg} = dataIn{pp, gg}.("UMean:2");
    ...
    wq = griddata(openFaomX{pp, indexOfPlane}/D, openFaomZ{pp, indexOfPlane}/D, ...
        openFaomWMean{pp, indexOfPlane}/V0,xq,zq);

    vq = griddata(openFaomX{pp, indexOfPlane}/D, openFaomZ{pp, indexOfPlane}/D, ...
        openFaomVMean{pp, indexOfPlane}/V0,xq,zq);

    uq = griddata(openFaomX{pp, indexOfPlane}/D, openFaomZ{pp, indexOfPlane}/D, ...
        openFaomUMean{pp, indexOfPlane}/V0,xq,zq);

    interporlatedDataMean.uq = uq;
    interporlatedDataMean.vq = vq;
    interporlatedDataMean.wq = wq;

    save("./NREL_FXXXXX_5D_000_00025_copy/exportedCSVMean/yPlane_00D_mean.mat", ...
        "interporlatedDataMean");
    ...
    if (alwayPlotting == 1 || indexCSVHolder == indexOfCSVfileArray(end))
        ...
        figureSizeSet = [100 500 1550 430];
        FigMaster = figure;
        ...
        quiverU = griddata(openFaomX{pp, indexOfPlane}/D, openFaomZ{pp, indexOfPlane}/D, ...
            openFaomUMean{pp, indexOfPlane}/V0 * quiverFactor,quiverXq,quiverZq);
        quiverW = griddata(openFaomX{pp, indexOfPlane}/D, openFaomZ{pp, indexOfPlane}/D, ...
            openFaomWMean{pp, indexOfPlane}/V0 * quiverFactor,quiverXq,quiverZq);
...
%         hcolor.FontSize = colorbar_font_size;
        ...
        zlabel('$z/D$','Interpreter','latex','FontSize',lable_font_size,'fontWeight','bold');
...
%         zLabelz.Position = [-3.1000    1.0    0.3072];
        ...
        if saveFigure == 1
            exportgraphics(FigMaster,"./NREL_FXXXXX_5D_000_00025_copy/exportedCSVMean/yPlane_00D_mean.png")
            %     saveas(FigMaster,figureFileNamePDF,'pdf')
            %     saveas(FigMaster,figureFileNameEPSC,'epsc')
        end
